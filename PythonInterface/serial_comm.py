...
class Controller:
    ...
    def pos_to_string(self, p):

        # string must look like  "iii iii iii i". it must be 13 characters long, all angles will be 1-3 digits, so we need to pad with zeros

        # one liner
        return " ".join(     [str(i).rjust(3, "0") for i in p] + [str(int(self.mag))]  )
    ...
    def connect(self):
        ...
        if len(options) > 1:
            ...
            print(f"Connecting to {options[choice]}")
    ...
    def send_signal(self, a1, a2, a3, mag):
        ...
        try:
            ...
            angle_str = self.pos_to_string([min(a1,180), min(a2,180), min(a3,184)])
